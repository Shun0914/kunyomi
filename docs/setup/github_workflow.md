...
3. 該当するラベルを選択：
   ...
   **ステータス（必須）**:
   - **`⛰️ Uphill`**: 悩み中（やり方を模索している、設計を考えている、エラーにハマっている状態）
   - **`⛷️ Downhill`**: 作業中（やることは明確で、あとは手を動かすだけの状態）

**例**: Issue #7「[フェーズ1] ドキュメント一覧取得API」の場合
- `backend`（カテゴリ）、`task`（種類）、`⛰️ Uphill`（ステータス、デフォルト）を選択
